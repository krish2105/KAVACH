...
def speech_bounds(audio: np.ndarray) -> tuple[int, int] | None:
    """First and last sample of speech, or None if the take is silent.

    Energy against the take's own noise floor rather than a fixed level: the
    same threshold has to work in a quiet room at night and a noisy one during
    the day, and a fixed number is wrong in one of them.
    """
    ...
    return int(voiced[0]) * _FRAME, int(voiced[-1] + 1) * _FRAME
